...
class PPW_Public {
	...
	private function ppw_free_content_filter( $content, $post_id ) {
		...
		return '<p><strong>' . esc_html__('This page is protected. Please try again or contact the website owner.', 'password-protect-page') . '</strong></p>';
	}
	...
	public function ppw_validate_password() {
		if ( isset( $_POST['nonce'] ) && ! wp_verify_nonce( $_POST['nonce'], 'ppw_password_nonce' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			wp_send_json(
				array(
					'success' => false,
